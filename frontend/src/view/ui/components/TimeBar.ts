...
export class TimeBar {
  ...
  private draw() {
    ...
    if (this.sunAltitudes.length > 0) {
      ...
      const step = 1 / (this.sunAltitudes.length - 1);
      
      for (let i = 0; i < this.sunAltitudes.length; i++) {
        const alt = this.sunAltitudes[i] ?? 0;
        grad.addColorStop(i * step, this.getAltitudeColor(alt));
      }
  ...
  private getAltitudeColor(alt: number): string {
    if (alt < -18) return "#050811"; // Deep night
    if (alt < -12) return "#0a1020"; // Astronomical twilight
    if (alt < -6) return "#151b30";  // Nautical twilight
    if (alt < -2) return "#252845";  // Civil twilight
    if (alt < 0) return "#5b3749";   // Pre-sunrise / Post-sunset
    if (alt < 2) return "#b85c49";   // Sunrise / Sunset (Orange/Red)
    if (alt < 6) return "#cca16e";   // Golden hour
    if (alt < 15) return "#6692b3";  // Morning/Evening sky
    return "#3b729e";                // Day sky
  }
