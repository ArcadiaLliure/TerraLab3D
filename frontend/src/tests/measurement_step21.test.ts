...
const normalCore = parent.getObjectByName("measurement:a:core") as any;
const normalHalo = parent.getObjectByName("measurement:a:halo") as any;
assert(normalCore.material.color.getHex() === 0xffffff, "normal measurement core is white");
assert(normalHalo.material.linewidth > (normalCore.material.coreWidth ?? 0), "normal measurement reuses one geometry with a wider halo");
assert(normalHalo.geometry === normalCore.geometry, "measurement core and halo share the retained geometry");
...
const selectedCore = parent.getObjectByName("measurement:a:core") as any;
const selectedHalo = parent.getObjectByName("measurement:a:halo") as any;
assert(selectedCore.material.color.getHex() === 0xffd24a, "selected measurement core is yellow");
renderer.updateTime(0);
const pulseOpacity = selectedHalo.material.opacity;
renderer.updateTime(Math.PI * 180);
assert(selectedHalo.material.opacity !== pulseOpacity, "measurement selection owns a gentle halo pulse");
assert(afterOneEdit.activeMaterialCount === 7, "measurement lines share three core/halo pairs and one handle material");
