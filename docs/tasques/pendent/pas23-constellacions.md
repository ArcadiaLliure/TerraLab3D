...
\- [TerraLab \`constellation\_drawing.py\`, commit auditat]\(https\://github.com/ArcadiaLliure/TerraLab/blob/1fbcf088a0bfc1f832fc0f2a8ba2808e3e783a7d/TerraLab/widgets/constellation\_drawing.py): oracle autoritzat del flux d'edició, no arquitectura ni dependència de runtime; consultat el 2026-09-17.
...
\- [Pas 13 — Picking, selecció i inspecció]\(../completat/pas13.md).
...
Les comandes versionades seran \`create\_group\`, \`append\_node\`, \`finish\_group\`, \`resume\_from\_node\`, \`select\`, \`rename\_group\`, \`delete\_selection\`, \`undo\`, \`redo\` i \`clear\`, amb \`requestId\` i \`expectedRevision\`. \`append\_node\` només transportarà \`StarPickRef { resourceId, resourceVersion, catalogIndex }\`; mai RA/Dec del frontend.
...
El document es desarà atòmicament a \`resolve\_data\_root()/constellations.v1.json\`. La migració de \`resolve\_data\_root()/terralab\_constellations.json\` serà idempotent i no modificarà l'original. Un document invàlid es copiarà amb nom datat abans d'inicialitzar un document buit vàlid.
...
Abans de modificar materials o shaders, cal inventariar tots els punts del frontend que dibuixen línies, polilínies, contorns o formes equivalents a l'escena. Com a mínim s'han de revisar el traçat adoptat de constel·lacions IAU, els grups de constel·lacions d'usuari, les formes de mesura angular, els contorns o formes angulars dels objectes NGC i les trajectòries aparents dels objectes. La cerca no es limitarà a aquesta llista: qualsevol altre renderer lineal existent s'haurà de classificar i documentar breument amb el component propietari, la tecnologia/material emprat i el lloc on es defineixen color, opacitat, gruix i efectes.

A l'últim commit auditat (`64a147c86147da342827487b4e32daff5d4b1686`), les trajectòries aparents es renderitzen a `frontend/src/view/three/ApparentTrajectoryRenderer.ts`. El renderer conserva una línia llegada amb `THREE.LineBasicMaterial` i, per al contracte segmentat, crea `THREE.LineSegments` per `TrajectoryVisibilityState`. Els estats actuals són `visible`, `terrain_occluded`, `below_astronomical_horizon` i `insufficient_data`; la seva codificació cromàtica, opacitat, `depthTest`, patró continu/discontinu i visibilitat són semàntica pròpia de la trajectòria i **no s'han d'aplanar** en centralitzar l'estil.
...
L'estil es centralitzarà; **els efectes i els estats continuaran pertanyent a cada funcionalitat**. La capa comuna podrà saber com representar una línia lluminosa, però no decidirà quan una mesura està seleccionada, quan ha de pulsar, quin tipus NGC determina un color, quin document de constel·lació és editable ni quin `TrajectoryVisibilityState` correspon a cada segment d'una trajectòria. `ApparentTrajectoryRenderer` continuarà sent responsable de la segmentació, del patró continu/discontinu, de la visibilitat de capes i de la semàntica d'oclusió/horizó. No es crearà un gestor monolític de selecció, animació i semàntica.
...
\- **Trajectòries aparents dels objectes:** s'integraran en el mateix llenguatge visual de nucli definit i halo subtil, però conservaran la semàntica actual per segments. El segment `visible` serà el traç principal i tindrà una presència lluminosa clara. `terrain_occluded` i `below_astronomical_horizon` mantindran la seva diferenciació cromàtica i el patró discontinu actual, així com els seus controls de visibilitat; `insufficient_data` conservarà una aparença diferenciada d'advertiment. L'estil compartit podrà aportar variants d'intensitat/halo, però no substituirà `stateStyle`, la classificació dels segments ni la decisió de quins segments es mostren. No s'afegirà pulsació a la trajectòria si no respon a un estat funcional explícit.
...
La jerarquia visual continuarà sent **cel i objectes astronòmics → informació seleccionada o interactiva → overlays permanents**. Una mesura seleccionada podrà destacar clarament més que una constel·lació persistent; una trajectòria activa haurà de ser fàcil de seguir sense competir amb el marcador temporal, els esdeveniments d'alba/posta o les etiquetes; i els contorns NGC tindran una intensitat més continguda.
...
\- [x] Ampliar el resolver autoritatiu perquè \`append\_node\` només accepti una \`StarPickRef\` vàlida i fixi coordenades i \`sourceId\` al backend.
...
\- [x] Inventariar tots els renderers lineals o de contorn de l'escena —constel·lacions IAU i d'usuari, mesura angular, NGC i qualsevol altre cas existent— i documentar tecnologia/material, propietari i paràmetres visuals actuals.

\- [x] Dissenyar i implementar l'abstracció compartida d'estil per a overlays lineals, separada de la lògica de selecció, animació i semàntica de cada funcionalitat.

\- [x] Integrar `ApparentTrajectoryRenderer` amb l'estil compartit sense alterar els `TrajectoryVisibilityState`, els colors semàntics actuals, els patrons continus/discontinus, els controls de segments ocults ni la persistència/reutilització de geometries i materials.

\- [x] Aplicar el llenguatge visual de nucli definit i halo subtil: constel·lacions adoptades blanques, constel·lacions d'usuari verdes, mesura angular normal blanca, mesura seleccionada groga i pulsant, i NGC amb els colors actuals per tipus però intensitat moderada.

\- [x] Aplicar el mateix llenguatge visual a les trajectòries aparents: `visible` com a traç lluminós principal i la resta d'estats amb intensitat/halo adaptats, sense perdre la codificació semàntica existent.

\- [x] Verificar que l'efecte no depèn d'un bloom global que alteri el cel i que la implementació comparteix recursos sempre que sigui possible sense trencar els estats dinàmics.
...
Visualment, els overlays lineals comparteixen una identitat coherent de nucli definit i halo subtil sense compartir necessàriament els seus efectes funcionals: el traçat adoptat de constel·lacions IAU és blanc, els grups d'usuari són verds, les mesures angulars normals són blanques i les seleccionades són grogues i pulsen, els contorns NGC conserven la codificació cromàtica per tipus amb una intensitat més moderada i les trajectòries aparents adopten el mateix llenguatge lluminós sense perdre la diferenciació entre `visible`, `terrain_occluded`, `below_astronomical_horizon` i `insufficient_data`. La nova capa comuna és extensible i no degrada perceptiblement ni la lectura del cel ni el rendiment.
...
\- [ ] Capturar una trajectòria aparent amb el nou estil i, en una escena de prova que ho permeti, verificar els estats `visible`, `terrain_occluded`, `below_astronomical_horizon` i `insufficient_data`; resultat esperat: el glow millora la llegibilitat sense confondre la semàntica ni tapar el marcador temporal, els esdeveniments o les etiquetes.
...
\- [ ] Verificar específicament que l'adaptació de trajectòries no introdueix reconstruccions per tick ni multiplica recursos per mostra; les geometries segmentades continuen sent persistents i només l'estat visual necessari s'actualitza.
