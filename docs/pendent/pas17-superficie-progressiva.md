# Pas 17 — Superfície, ortofoto i càrrega visual progressiva

> Estat: **parcial**. La cobertura categòrica és funcional; falten l'ortofoto, els estils complets i la progressió visual de totes les fonts de superfície.
...
- [ ] El port d'ortofoto i el seu adaptador continuen sent esquelets.
- [ ] El canvi original/vibrant i el refinament progressiu de superfície encara no compleixen la vertical.

## Resultat funcional

L'usuari alterna entre terreny tècnic, ortofoto i cobertura categòrica original o vibrant. La geometria no es reconstrueix en canviar d'estil i una ubicació nova mostra primer una superfície basta que es refina per tiles sense bloquejar la càmera.
...
- RGB es remostreja segons la seva naturalesa contínua; classes categòriques sempre amb veí més proper.
...
- Contracte de terreny/ortofoto: [`terrain.py`](../../backend/src/terralab3d/application/ports/terrain.py).
...
- Oracle TerraLab: [servei de superfície](https://github.com/ArcadiaLliure/TerraLab/blob/1fbcf088a0bfc1f832fc0f2a8ba2808e3e783a7d/TerraLab/terrain/surface/service.py), [RGB](https://github.com/ArcadiaLliure/TerraLab/blob/1fbcf088a0bfc1f832fc0f2a8ba2808e3e783a7d/TerraLab/terrain/surface/rgb.py) i [categories](https://github.com/ArcadiaLliure/TerraLab/blob/1fbcf088a0bfc1f832fc0f2a8ba2808e3e783a7d/TerraLab/terrain/surface/categorical.py).

## Treball pendent

- [ ] Implementar un port d'ortofoto real amb CRS, resolució, `nodata`, procedència i errors tipats.
- [ ] Publicar textura RGB o atributs categòrics versionats per tile i descartar respostes de revisions antigues.
- [ ] Afegir modes terreny/ortofoto/categòric i estils original/vibrant a la UI.
...
- [ ] Reutilitzar el mateix protocol progressiu per ortofoto i cobertura, amb pressupostos de memòria CPU/GPU.
- [ ] Alliberar textures substituïdes, cancel·lar treball pendent en canviar d'ubicació i conservar la càmera interactiva.

## Flux tècnic

`SetSurfaceMode` o canvi de font → coordinador de superfície → port RGB/categòric → tile versionat → bridge binari → gestor de textura retinguda → actualització del material existent. Un canvi només de paleta entra directament al gestor de material.
...
- Numèriques: CRS, veí més proper categòric, RGB, `nodata`, ordre de fallback i revisió obsoleta.
...
Els quatre modes visuals són utilitzables; les dades informen honestament de font i qualitat; l'estil no reconstrueix la malla; la superfície apareix i es refina progressivament amb memòria acotada.

## Evidències

- [ ] Captures de terreny, ortofoto, categòric original i vibrant.
...
Descoberta de refinadors semàntics, TLST i AOI avançada ([pas 29](pas29-superficie-semantica.md)); meteorologia ([pas 18](pas18-meteorologia.md)).
