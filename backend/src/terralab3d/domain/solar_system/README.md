# Capacitat `solar_system` — Sol, Lluna i planetes

## 1. Propòsit

Representar estats aparents dels cossos, fases, diàmetres angulars, il·luminació i trajectòries.
...
- **Entrades:** Efemèrides, observador, temps i paràmetres de precisió.
- **Sortides:** Estats topocèntrics aparents i propietats fotomètriques.
...
|---|---|---|---|
| `ApparentBodyState` | Entitat | Estat aparent d’un cos | Esquelet |
| `SolarSystemCalculator` | Protocol científic | Posició, fase i diàmetre | Esquelet |
| `SolarSystemModel` | Servei | Composició des d’efemèrides | Esquelet |
...
|---|---|---|---|---|---|---|---|
| `TerraLab/astro/engine.py` | `Meeus/ELP/òrbites i topocentrisme` | Lògica i comportament actuals | Regles científiques, invariants, fixtures i semàntica útils | Qt, QPainter, diccionaris sense tipus, threads o I/O que no pertanyin al domini | Aïllar, tipar i caracteritzar abans de traslladar | `backend/src/terralab3d/domain/solar_system` | `EXTRACT` |
| `TerraLab/ephemeris_coordinator.py` | `coordinació de proveïdors` | Lògica i comportament actuals | Regles científiques, invariants, fixtures i semàntica útils | Qt, QPainter, diccionaris sense tipus, threads o I/O que no pertanyin al domini | Aïllar, tipar i caracteritzar abans de traslladar | `backend/src/terralab3d/domain/solar_system` | `ADAPT` |
