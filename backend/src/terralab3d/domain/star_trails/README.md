# Capacitat `star_trails` — traces circumpolars

## 1. Propòsit

Modelar intervals d’exposició, mostreig temporal i geometria esfèrica de traces sense acumular frames raster.
...
- **Entrades:** Catàleg seleccionat, observador, interval temporal, exposició i política de mostreig.
- **Sortides:** Segments de traces i descriptors persistents actualitzables.
...
|---|---|---|---|
| `StarTrailRequest` | DTO | Interval i paràmetres de trace | Esquelet |
| `StarTrailGeometry` | DTO | Segments científics de la trace | Esquelet |
| `StarTrailCalculator` | Protocol científic | Mostreig temporal i arcs | Esquelet |
...
|---|---|---|---|---|---|---|---|
| `TerraLab/runtime/offscreen_renderer.py; camins overlay circumpolars` | `acumulació lligada a frames raster` | Lògica i comportament actuals | Regles científiques, invariants, fixtures i semàntica útils | Qt, QPainter, diccionaris sense tipus, threads o I/O que no pertanyin al domini | Aïllar, tipar i caracteritzar abans de traslladar | `backend/src/terralab3d/domain/star_trails` | `REWRITE` |
