# Capacitat `navigation` — càmera i navegació 360°

## 1. Propòsit

Modelar intencions de navegació, orientació, zoom, camp visual, seguiment i enfocament d’objectes.
...
- **Entrades:** Gestos normalitzats, objectiu, velocitat i estat actual de càmera.
- **Sortides:** Pose de càmera i intencions de seguiment independents del renderer.
...
|---|---|---|---|
| `CameraPose` | Entitat | Orientació i camp visual | Esquelet |
| `CameraNavigationState` | Agregat | Estat i seguiment | Esquelet |
| `CameraIntentModel` | Servei | Aplicació pura d’intencions | Esquelet |
...
|---|---|---|---|---|---|---|---|
| `TerraLab/scene/camera.py` | `azimut, elevació, zoom i FOV llegat` | Lògica i comportament actuals | Regles científiques, invariants, fixtures i semàntica útils | Qt, QPainter, diccionaris sense tipus, threads o I/O que no pertanyin al domini | Aïllar, tipar i caracteritzar abans de traslladar | `backend/src/terralab3d/domain/navigation` | `ADAPT` |
| `TerraLab/ui/canvas_mixins/interaction.py` | `gestos i navegació` | Lògica i comportament actuals | Regles científiques, invariants, fixtures i semàntica útils | Qt, QPainter, diccionaris sense tipus, threads o I/O que no pertanyin al domini | Aïllar, tipar i caracteritzar abans de traslladar | `backend/src/terralab3d/domain/navigation` | `REWRITE` |
