# Capacitat `science` — fonaments científics compartits

## 1. Propòsit

Definir unitats, convencions, precisió numèrica, incertesa, èpoques i tipus vectorials compartits pels càlculs científics.
...
- **Entrades:** Valors escalars, vectors, instants, unitats i toleràncies explícites.
- **Sortides:** Tipus científics validables i contractes numèrics comuns.
...
|---|---|---|---|
| `Angle` | Tipus de valor | Angle amb unitat i domini explícits | Esquelet |
| `ScientificTolerance` | DTO | Toleràncies absolutes i relatives | Esquelet |
| `ScientificComputationContext` | DTO | Època, precisió i convencions de càlcul | Esquelet |
...
|---|---|---|---|---|---|---|---|
| `TerraLab/astro/engine.py; TerraLab/scene/projection.py; TerraLab/widgets/spherical_math.py` | `constants, conversions i helpers dispersos` | Lògica i comportament actuals | Regles científiques, invariants, fixtures i semàntica útils | Qt, QPainter, diccionaris sense tipus, threads o I/O que no pertanyin al domini | Aïllar, tipar i caracteritzar abans de traslladar | `backend/src/terralab3d/domain/science` | `EXTRACT` |
