# Capacitat `horizon` — horitzó topogràfic

## 1. Propòsit

Calcular perfils d’horitzó, oclusió celeste, capes de distància i resolució angular.
...
- **Entrades:** Observador, mostres DEM, radi visible i pas angular.
- **Sortides:** Perfil d’horitzó i consultes d’oclusió.
...
|---|---|---|---|
| `HorizonRequest` | DTO | Paràmetres del perfil | Esquelet |
| `HorizonProfile` | Entitat | Mostres ordenades | Esquelet |
| `HorizonCalculator` | Protocol científic | Raycast i oclusió | Esquelet |
...
|---|---|---|---|---|---|---|---|
| `TerraLab/terrain/worker.py; TerraLab/terrain/terrain_coordinator.py` | `bake, threads i perfil` | Lògica i comportament actuals | Regles científiques, invariants, fixtures i semàntica útils | Qt, QPainter, diccionaris sense tipus, threads o I/O que no pertanyin al domini | Aïllar, tipar i caracteritzar abans de traslladar | `backend/src/terralab3d/domain/horizon` | `EXTRACT` |
| `TerraLab/render/horizon_renderer.py` | `pintura de siluetes` | Lògica i comportament actuals | Regles científiques, invariants, fixtures i semàntica útils | Qt, QPainter, diccionaris sense tipus, threads o I/O que no pertanyin al domini | Aïllar, tipar i caracteritzar abans de traslladar | `backend/src/terralab3d/domain/horizon` | `DISCARD` |
