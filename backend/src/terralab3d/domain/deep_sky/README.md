# Capacitat `deep_sky` — objectes de cel profund

## 1. Propòsit

Modelar NGC/IC, classificació, dimensions aparents, magnitud, visibilitat i atributs de cerca.
...
|---|---|---|---|
| `DeepSkyObject` | Entitat | Objecte NGC/IC | Esquelet |
| `DeepSkyVisibilityCalculator` | Protocol científic | Visibilitat superficial i integrada | Esquelet |
| `DeepSkyVisibilityModel` | Servei | Selecció científica | Esquelet |
...
|---|---|---|---|---|---|---|---|
| `TerraLab/astro/ngc_catalog.py` | `parser, normalització i tipus NGC` | Lògica i comportament actuals | Regles científiques, invariants, fixtures i semàntica útils | Qt, QPainter, diccionaris sense tipus, threads o I/O que no pertanyin al domini | Aïllar, tipar i caracteritzar abans de traslladar | `backend/src/terralab3d/domain/deep_sky` | `ADAPT` |
| `TerraLab/runtime/offscreen_renderer.py` | `filtrat i dibuix` | Lògica i comportament actuals | Regles científiques, invariants, fixtures i semàntica útils | Qt, QPainter, diccionaris sense tipus, threads o I/O que no pertanyin al domini | Aïllar, tipar i caracteritzar abans de traslladar | `backend/src/terralab3d/domain/deep_sky` | `EXTRACT` |
