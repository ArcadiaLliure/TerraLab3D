# Capacitat `optics` — telescopi, ocular i geometria òptica

## 1. Propòsit

Calcular camp visual, escala, pupil·la de sortida, augment i geometria d’enquadrament d’instruments.
...
- **Entrades:** Focal, obertura, ocular, sensor, relació d’aspecte i forma.
- **Sortides:** Camp visual, augment, escala angular i màscara neutral.
...
|---|---|---|---|
| `OpticalInstrument` | Entitat | Configuració òptica | Esquelet |
| `FieldOfView` | DTO | Camp angular | Esquelet |
| `OpticalGeometryCalculator` | Protocol científic | FOV, augment i escala | Esquelet |
...
|---|---|---|---|---|---|---|---|
| `TerraLab/widgets/telescope_scope_mode.py` | `presets, FOV, gestos i QPainter` | Lògica i comportament actuals | Regles científiques, invariants, fixtures i semàntica útils | Qt, QPainter, diccionaris sense tipus, threads o I/O que no pertanyin al domini | Aïllar, tipar i caracteritzar abans de traslladar | `backend/src/terralab3d/domain/optics` | `EXTRACT` |
| `TerraLab/ui/widget_controls_builder.py` | `controls d’instrument` | Lògica i comportament actuals | Regles científiques, invariants, fixtures i semàntica útils | Qt, QPainter, diccionaris sense tipus, threads o I/O que no pertanyin al domini | Aïllar, tipar i caracteritzar abans de traslladar | `backend/src/terralab3d/domain/optics` | `ADAPT` |
