# Capacitat `photometry` — fotometria astronòmica compartida

## 1. Propòsit

Centralitzar magnituds, fluxos, luminància, resposta instrumental, color i llindars sense dependències de render.
...
- **Entrades:** Magnitud, espectre/color, atmosfera, instrument i exposició.
- **Sortides:** Flux relatiu, senyal, saturació, contrast i visibilitat.
...
|---|---|---|---|
| `PhotometricSample` | DTO | Magnitud i flux normalitzats | Esquelet |
| `PhotometryCalculator` | Protocol científic | Conversions de magnitud i flux | Esquelet |
| `VisibilityContrastModel` | Servei | Llindars de detecció | Esquelet |
...
|---|---|---|---|---|---|---|---|
| `TerraLab/visual_magnitude_engine.py; TerraLab/physical_math.py; TerraLab/render/stars_renderer.py` | `regles fotomètriques disperses` | Lògica i comportament actuals | Regles científiques, invariants, fixtures i semàntica útils | Qt, QPainter, diccionaris sense tipus, threads o I/O que no pertanyin al domini | Aïllar, tipar i caracteritzar abans de traslladar | `backend/src/terralab3d/domain/photometry` | `EXTRACT` |
